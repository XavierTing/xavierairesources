...
- **Artwork:** every themed image ships in two inkings, swapped by theme.js — the AI 101 art (`assets/101/light/` original linen, `assets/101/` recoloured dark) and the card monolines (`assets/cards/light/`, generated by `assets/cards/lighten.py`: strokes re-inked #6F5631 with alpha lifted 2.2× — the Night set's low stroke alpha is tuned to glow on near-black and no CSS filter survives it on linen). The markup ships the light paths, so the swap only fires for Night. The stack map and og-image stay dark renders in both themes (light re-renders are open items).
...
Index: hero (headline → sub → byline → counts) → tabs row on a hairline → card grid (4 cols ≥1600px, 3 by default, 2 at ≤1020px, 1 at ≤640px; 20px gap, 14px at ≤640px) → stack map → colophon. Detail (tool pages and the curator): breadcrumb/strip → two-column header (title + lede left, 300px meta panel right; stacks at ≤1020px) → full-width sections (110px label column, stacks at ≤640px) → colophon. Course: sticky chapter bar → chapter plate → entries, which split into prose + analogy/diagram rail (7fr/5fr) at ≥1240px and stack below. 8px rhythm; generous vertical clamp spacing.
