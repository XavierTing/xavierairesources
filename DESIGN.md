...
- **Artwork:** every themed image ships in two inkings, swapped by theme.js — the AI 101 art (`assets/101/light/` original linen, `assets/101/` recoloured dark) and the card monolines (`assets/cards/light/`, generated by `assets/cards/lighten.py`: strokes re-inked #6F5631 with alpha lifted 2.2× — the Night set's low stroke alpha is tuned to glow on near-black and no CSS filter survives it on linen). The markup ships the light paths, so the swap only fires for Night. og-image is a Daylight render from `og/og-image.html`, count-free so it cannot go stale, re-rendered only when the identity line changes.
...
- **Nav** (mono, 1rem/16px, +0.08em, uppercase): the masthead strip, the section tabs, the category filters and their count, breadcrumbs, back/pager links, and the course's chapter bar. Wayfinding is read before anything else, so it sits a step above the label tier — added 2026-08-12 after the shared label step proved too dense for it. The section nav lives in the masthead (see Components): tracked mono links at the label step beside the Montserrat wordmark.
